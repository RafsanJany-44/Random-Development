...
CREATE OR REPLACE FUNCTION msg.chat_group_add(p_json text)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
declare
...
begin
	...
	perform msg.chat_group_member_add(temp4_json);
	return_json= json_build_object('code', 1);
	raise notice 'Value: %', return_json;
	return return_json;
end;
...
begin
	...
	select
		...
		discussion_class
	from
		json_populate_record(null::public.discussion_category , v_json) returning dc_id into v_dc_id;
